...
class NLTKprocessing(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, similarity_threshold, max_df,matches,number_of_matches,big_manufacturer_list,max_ngram):
    ...
    def TFIDF(self,X,filtre,max_ngram):
        logging.debug(">>>>>>>>NLTKProcessing->TFIDF() called.")
        logging.debug(f"max_ngram={self.max_ngram}")
        logging.debug(f"max_df={self.max_df}")
        vectorizer = TfidfVectorizer(ngram_range=(1,max_ngram), max_df=self.max_df,sublinear_tf=True,stop_words=[filtre])
    ...
    def prep(self,texte):
        ...
        texte = texte.replace("professional", "pro").replace(" win ", " windows ")\
            .replace(" upg "," upgrade ").replace(" dlx "," deluxe ")
    ...
    def generate_corpus(self,company1,company2):
        ...
        company1['full data'] = company1['manufacturer'].apply(self.prep) + ' ' + company1['name'].apply(self.prep)  # + ' ' + company1['description'].apply(prep)
        company2['Company'] = "company2"
        ...
        company2['full data'] = company2['manufacturer'].apply(self.prep) + ' ' + company2['name'].apply(self.prep)
    ...
    def run(self,company1,company2,corpus,filtre,similarity_threshold,max_ngram):
        new_number_of_matches = 0
        new_matches = []

        dense = self.TFIDF(corpus,filtre,max_ngram)
        self.dense = dense
        ...
        for i in tqdm(range(n1),position=0,desc="Global loop",leave=False,colour="green",ncols=80):
            ...
            for j in range(n2):
                ...
                if (price1 * price2 == 0 or max(price1, price2) / min(price1, price2) < 2) and (
                        company1.iloc[i, 11] == company2.iloc[j, 11] or company1.iloc[
                    i, 11] == 'None' or company2.iloc[j, 11] == 'None'):
                    try:
                        similarity = np.dot(dense[i], np.transpose(dense[len(company1) + j])).item(0) / math.sqrt(
                            np.dot(dense[i], np.transpose(dense[i])).item(0) * np.dot(dense[n1 + j],
                                                                                      np.transpose(dense[n1 + j])).item(
                                0))
                    except:
                        similarity = 0
    ...
    def fit(self, company1,company2):
        ...
        for filtre in self.big_manufacturer_list:
            ...
            for max_ngram in [2, 1]:
                ...
                self.run(company1_light,company2_light,corpus,filtre,self.similarity_threshold,max_ngram)
        ...
        for max_ngram in [3, 2, 1]:
            ...
            self.run(company1_light, company2_light, corpus, "", (self.similarity_threshold)/1.2,max_ngram)
...
if __name__ == "__main__":
    ...
    while s < Max:
        ...
        matches = []
        matches_df = []
        model = NLTKprocessing(0.6,max_df,matches,number_of_matches, big_manufacturer_list,max_ngram)
